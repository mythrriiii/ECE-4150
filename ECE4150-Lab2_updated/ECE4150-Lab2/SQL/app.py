...
table = dynamodb.Table('PhotoGalleryUser')
...
def signup():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        statement = f'''INSERT INTO photogallerydb.User (email, password, name, confirmed, userID ) VALUES ("{email}", "{password}", "{name}", "false", "{token}");'''
...
def confirm_email(token):
    ...
    if (email) :
        conn=get_database_connection()
        cursor=conn.cursor()
        statement = f'''UPDATE photogallerydb.User SET confirmed = 'true' WHERE email ="{email}";'''
        cursor.execute(statement)
        results= cursor.fetchall()
        conn.commit()
        conn.close()
...
def login():
    ...
    if request.method == 'POST':
        ...
        conn=get_database_connection()
        cursor=conn.cursor()
        statement = f'''SELECT email, password, confirmed FROM photogallerydb.User WHERE email ="{email}" AND confirmed="true";'''
        cursor.execute(statement)
        results= cursor.fetchall()
        conn.commit()
        conn.close()
...
def deleteAccount():
    #delete all photos within an album
    conn = get_database_connection()
    cursor = conn.cursor()
    statement = f'''SELECT * FROM photogallerydb.Photo;''' 
    cursor.execute(statement)
    
    for item in results:
        if item['creator'] == session['email']:  # Fixed the placeholder usage
            statement = f'''DELETE FROM photogallerydb.Photo WHERE photoID="{item['photoID']}" AND albumID="{item['albumID']}" AND creator="{item['creator']}";'''  
            cursor.execute(statement)   

    #delete all albums 
    statement = f'''DELETE FROM photogallerydb.Album WHERE creator="{session['email']}";''' 
    cursor.execute(statement)

    #delete user
    
    statement = f'''DELETE FROM photogallerydb.User WHERE email="{session['email']}";'''  # Used session email to delete the user
    cursor.execute(statement)
    results= cursor.fetchall()
    conn.commit()
    conn.close() 
...
def deletePhoto(albumID, photoID):
   ...
   conn = get_database_connection()
   cursor = conn.cursor()
   statement = f'''DELETE FROM photogallerydb.Photo WHERE albumID = %s AND photoID = %s;'''
...
def deleteAlbum(albumID):
    ...
    statement = f'''DELETE FROM photogallerydb.Photo WHERE albumID = %s;'''
    cursor.execute(statement, (albumID,))

    statement = f'''DELETE FROM photogallerydb.Album WHERE albumID = %s;'''
    cursor.execute(statement, (albumID,))
...
def updatePhoto(albumID, photoID):
    if request.method == 'POST':
        ...
        updateTime = updatedAtUTCTime.strftime("%Y-%m-%d %H:%M:%S")

        conn = get_database_connection()
        cursor = conn.cursor()


        statement = f'''UPDATE photogallerydb.Photo SET title = %s, description = %s, tags = %s, updatedAt = %s WHERE albumID = %s AND photoID = %s;'''
        cursor.execute(statement, (title, description, tags, updateTime, albumID, photoID))
